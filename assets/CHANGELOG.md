...
- feat(macos): include FileFlow as a share target in the share menu (@ShlomoCode)
...
- fix(windows): FileFlow window is invisible at app start (@Tienisto)
...
- feat: share via browser link (for non-FileFlow users)
...
- feat: implement FileFlow protocol v2 with v1 fallback
