...
import '../../model/dto/multicast_dto.dart';
import '../../model/dto/register_dto.dart';
import '../isolate/child/dio_provider.dart';
      ...
      socket.socket.listen((_) {
        ...
        try {
          ...
          final peer = dto.toDevice(
              ip, syncState.port, syncState.protocol == ProtocolType.https);
  ...
  Future<void> sendAnnouncement() async {
    ...
    for (final wait in [100, 500, 2000]) {
      ...
      for (final socket in sockets) {
        try {
          socket.socket.send(
              dto, InternetAddress(syncState.multicastGroup), syncState.port);
          socket.socket.close();
        } catch (e) {
