...
import '../../discovery/http_scan_discovery.dart';
import 'main.dart';
    ...
    handler: (task) async {
      final stream = switch (task.data) {
        HttpInterfaceScanTask data =>
          isolateContainer.read(httpScanDiscoveryProvider).getStream(
                networkInterface: data.networkInterface,
                port: data.port,
                https: data.https,
              ),
        HttpFavoriteScanTask data =>
          isolateContainer.read(httpScanDiscoveryProvider).getFavoriteStream(
